...
const ALL_DEPT_CARDS = [
  { path: '/icu',        label: 'ICU',        icon: '♥',  desc: 'Live vitals monitoring',     color: 'var(--accent-red)',   dept: 'icu' },
  { path: '/cardiology', label: 'Cardiology', icon: '⚡', desc: 'ECG reports & cardiac data', color: 'var(--accent-cyan)',  dept: 'cardiology' },
  { path: '/radiology',  label: 'Radiology',  icon: '◎', desc: 'Scan results & imaging',     color: '#a78bfa',             dept: 'radiology' },
  { path: '/neurology',  label: 'Neurology',  icon: '◈', desc: 'Neurological assessments',   color: '#fb923c',             dept: 'neurology' },
  { path: '/oncology',   label: 'Oncology',   icon: '✦', desc: 'Treatment plans & cycles',   color: '#34d399',             dept: 'oncology' },
  { path: '/admin',      label: 'Admin',      icon: '⊞', desc: 'Cross-department overview',  color: 'var(--text-muted)',   dept: 'admin' },
]
...
export default function Dashboard() {
  ...
  // Mirror the same access logic as Layout.tsx sidebar
  const DEPT_CARDS = isAdmin
    ? ALL_DEPT_CARDS
    : ALL_DEPT_CARDS.filter(c => c.dept === dept)
